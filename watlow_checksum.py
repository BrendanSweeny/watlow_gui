...
def header(hexCommand):
    hexCommand = unhexlify(hexCommand)
    crc_table = [
    ...
    ans = hex(~crc_table[hexCommand[6] ^ crc_table[hexCommand[5] ^ crc_table[hexCommand[4] ^ crc_table[hexCommand[3] ^ crc_table[~hexCommand[2]]]]]] & (2**8-1))
    return ans

print(header('55FF060010000B'))
